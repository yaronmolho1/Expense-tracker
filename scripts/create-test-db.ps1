...
# Usage: .\scripts\create-test-db.ps1
...
Write-Host "Creating test database: $TEST_DB_NAME" -ForegroundColor Cyan
...
Write-Host "Checking if test database exists..." -ForegroundColor Gray
...
if ($result -match "1") {
    Write-Host "[OK] Test database already exists: $TEST_DB_NAME" -ForegroundColor Green
} else {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "[OK] Test database created: $TEST_DB_NAME" -ForegroundColor Green
    } else {
